...
class pageStrategy {
  ...
  getPrevButton() {
    const polyline = document.querySelector(
      'polyline[points="1,2 5,6 9,2"][transform="rotate(90 5 4)'
    );
    // Traverse up to the <a> tag
    const aTag = polyline.closest("a");
    console.log(aTag);
    return aTag;
  }
  getNextButton() {
...
class artScraper {
  ...
  setStrategy() {
    ...
    if (url.includes("https://www.pixiv.net/bookmark_new_illust.php")) {
      ...
      url.match(/^https:\/\/www\.pixiv\.net\/(en\/users|users)\/.*\/.*$/)
    ) {
      return new userStrategy();
    } else if (
      url.match(/^https:\/\/www\.pixiv\.net\/(en\/tags|tags)\/.*\/.*$/)
    ) {
  ...
  async eatAllArts() {
    ...
    await this.executeAndcountUpSec("readingPages", () =>
      this.readingPages(
    ...
    await this.executeAndcountUpSec("renderArtWall", () =>
      this.renderArtWall(renderArtWallAtClass)
    );
    ...
    this.addRerenderButton(
      ...
      buttonAtClass,
      this.strategy.getAllButtonClass()[2]
    );
  ...
  async readingPages(thumbnailClass, artsClass) {
    ...
    for (let i = 0; i <= this.targetPages; i++) {
      ...
      // 最後一頁的下一頁按鈕為隱藏
      let allPageNav = document.querySelectorAll(
        "a.sc-xhhh7v-1-filterProps-Styled-Component"
      );
      ...
      if (this.allArtsWithoutLike.length >= 800) {
        while (this.allArtsWithoutLike.length != 0) {
          try {
            await this.executeAndcountUpSec("appendLikeElementToAllArts", () =>
              this.appendLikeElementToAllArts()
            );
          } catch (e) {
            console.log("請等待API冷卻時間");
            await this.delay(waitTime);
  ...
  async getArtsInPage(thumbnailClass, artsClass) {
    ...
    while (thumbnailCount < pageStandard) {
      ...
      if (thumbnailCount < pageStandard) {
        console.log(
          `缺少${
            pageStandard - thumbnailCount
          }張圖片，請關閉開發者工具且保持視窗在本分頁以確保所有圖片都載入`
        );
  ...
  async appendLikeElementToAllArts() {
    this.allArtsWithoutLike = this.allArtsWithoutLike.filter(
      (art) => art !== undefined
    );
    const ids = this.allArtsWithoutLike
      .filter((art) => art.getElementsByTagName("a")[0] !== undefined)
      .map((art) => {
    ...
    const likeCounts = await Promise.all(
      ids.map((id) => this.fetchLikeCount(id))
    );
  ...
  async toNextPage() {
    this.strategy.getNextButton().click();
  }

  async toPervPage() {
    this.strategy.getPrevButton().click();
  }
  ...
  async renderArtWall(renderArtWallAtClass) {
    const parentElement = await this.getElementOrListBySelector(
      renderArtWallAtClass
    );
    ...
    for (let art of this.allArts) {
      if (
        art.getElementsByClassName("likes")[0].textContent >= this.likesMinLimit
      ) {
  ...
  async addStartButton(ParentClass, buttonClass) {
    ...
    const buttonContainer = this.createButtonContainer(
      "nav",
      "myScriptButtonContainer",
      "fHLYsP"
    );
    const startButton = this.createButton(buttonClass, "StartButton", "likes:");
    startButton.style.lineHeight = "0.5rem";
    ...
    (await this.getElementOrListBySelector(ParentClass)).appendChild(
      buttonContainer
    );
  }
  ...
  async addRerenderButton(renderArtWallAtClass, ParentClass, buttonClass) {
    if (document.getElementById("RerenderButton")) {
      this.updateRerenderButtonText();
      return;
    }

    this.hideElements(
      "nav#myScriptButtonContainer input[id=LikeRangeInput]",
      "#LikeIcon"
    );
    document.getElementById("StartButton").textContent =
      "bug? you can try again.";

    const buttonContainer = this.createButtonContainer("div");
    const rerenderButton = this.createButton(
      buttonClass,
      "RerenderButton",
      "likes:"
    );
  ...
  createButtonContainer(elementType, id, className) {
    const container = document.createElement(elementType);
    container.style.display = "flex";
    container.style.alignItems = "center";
    if (id) container.id = id;
    if (className) container.classList = className;
    return container;
  }

  createButton(buttonClass, id, textPrefix) {
    const button = document.createElement("button");
    button.textContent = `${textPrefix} ${this.likesMinLimit} for ${this.targetPages}Page Go!`;
    button.classList.add(
      ...
      "BSrHG",
      "bRXHR",
      "gtm-my-tag-add-button"
    );
    button.id = id;
    return button;
  }

  updateRerenderButtonText() {
    const rerenderButton = document.getElementById("RerenderButton");
    rerenderButton.textContent = `likes: ${
      this.likesMinLimit
    } Rerender Go! now:${this.currentArtCount}(${Math.round(
      (this.currentArtCount / this.allArts.length) * 100
    )}％)`;
  }
  ...
  addLikeRangeInput(container, button) {
    ...
    const likeRangeInput = this.createLikeRangeInput(button);

    container.append(likeIcon, likeRangeInput);
  }
  ...
  createLikeIcon() {
    ...
    likeIcon.innerHTML = `
            ...
            class="sc-j89e3c-0 dUurgf"
            fill="var(--charcoal-text2)"
            stroke="var(--charcoal-text2)">
            </path>`;
    return likeIcon;
  }

  createLikeRangeInput(button) {
    const likesMinLimitsRange = [
      0, 10, 25, 50, 100, 250, 500, 1000, 2500, 5000, 7500, 10000,
    ];
    ...
    const updateText = (prefix) =>
      `${prefix}: ${this.likesMinLimit} for ${this.targetPages}Page Go!`;

    likeRangeInput.addEventListener("input", (event) => {
      this.likesMinLimit = likesMinLimitsRange[event.target.value];
      button.textContent = updateText("likes");
    });

    return likeRangeInput;
  }
  async addPageRangeInput(container, startButton) {
    const pageIcon = document.createElementNS(
      "http://www.w3.org/2000/svg",
      "svg"
    );
    ...
    const max = (await this.getMaxPage()) || 34;
    const stepSize = Math.floor(max / 24);
    const initialPageValue = Math.min(this.targetPages, max);
    ...
    const syncInputs = (value) => {
      ...
      startButton.textContent = `likes: ${this.likesMinLimit} for ${value} Page Go!`;
    };

    const pageRangeInput = createInput("range", initialPageValue);
    ...
    const pageInputBox = createInput("number", initialPageValue);
    pageInputBox.style.height = "0.4rem";
    pageInputBox.style.marginRight = "10px";
    pageInputBox.classList.add("sc-5ki62n-4", "eOTMOA");
    pageInputBox.addEventListener("input", (event) => {
      const value = parseInt(event.target.value);
      if (value >= 1 && value <= max) syncInputs(value);
    });

    container.append(pageIcon, pageRangeInput, pageInputBox);
  }
  ...
  async getMaxPage() {
    ...
    const artsCountElement = await this.getElementOrListBySelector(
      this.strategy.getArtsCountClass()
    );
    ...
    if (artsCountElement) {
      ...
      const arts = await this.getElementOrListBySelector(
        this.strategy.getArtsClass()
      );
...
class customMenu {
  ...
  rowsOfArtsWallMenu() {
    const rows = parseInt(
      prompt(
        `${this.getFeatureMessageLocalization(
          "rowsOfArtsWallPrompt"
        )} ${this.getRowsOfArtsWall()}`
      )
    );
  ...
  getFeatureMessageLocalization(word) {
    let display = {
      "zh-TW": {
        rowsOfArtsWall: "行數設定",
        rowsOfArtsWallPrompt:
          "一行顯示幾個繪畫?(請根據瀏覽器放大程度決定) 目前為：",
        rowsOfArtsWallMenuError: "請輸入一個數字，且不能小於1",
      },
      en: {
        rowsOfArtsWall: "row setting",
        rowsOfArtsWallPrompt:
          "How many paintings should be displayed in one row?(Please decide based on browser magnification level) Currently:",
        rowsOfArtsWallMenuError:
          "Please enter a number, and it cannot be less than 1",
      },
      ja: {
        rowsOfArtsWall: "行設定",
        rowsOfArtsWallPrompt:
          "1 行に何枚の絵畫を表示する必要がありますか?(ブラウザの倍率レベルに基づいて決定してください) 現在：",
        rowsOfArtsWallMenuError:
          "數値を入力してください。1 未満にすることはできません",
      },
    };
  ...
  registerMenuCommand(instance) {
    ...
    GM_registerMenuCommand(
      instance.getFeatureMessageLocalization("rowsOfArtsWall"),
      () => instance.rowsOfArtsWallMenu()
    );
  }
}
...
class readingStand {
  static expandAllArtworks() {
    ...
    if (
      artistHomePattern.test(self.location.href) ||
      tagHomePattern.test(self.location.href)
    ) {
...
const observer = new MutationObserver(function (mutations) {
  mutations.forEach(function (mutation) {
    ...
    johnTheHornyOne.addStartButton(
      johnTheHornyOne.strategy.getButtonAtClass(),
      johnTheHornyOne.strategy.getAllButtonClass()[0]
    );
  });
});
